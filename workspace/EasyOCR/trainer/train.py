...
def train(opt, show_number=2, amp=False, wandb_run=None):
    """ Подготовка датасета """
    ...
    while(True):
        ...
        if (i % opt.valInterval == 0) and (i != 0):
            ...
            with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a', encoding="utf8") as log:
                ...
                # Отображение некоторых предсказанных результатов
                dashed_line = '-' * 80
                ...
                if wandb_run:
                    wandb_run.log({
                        ...
                        "Best Norm ED": best_norm_ED
                    })

        # Сохранение модели каждые 10,000 итераций
        if (i + 1) % 1e+4 == 0:
            torch.save(
                model.state_dict(), f'./saved_models/{opt.experiment_name}/iter_{i+1}.pth')
            if wandb_run:
                wandb_run.save(f'./saved_models/{opt.experiment_name}/iter_{i+1}.pth')
